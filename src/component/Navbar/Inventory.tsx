...
const Inventory = ({ onlyVerified = false }: { onlyVerified?: boolean }) => {
    ...
    setTimeout(() => {
      setInventory((prevInventory) =>
        prevInventory.map((item) =>
          item.id === itemToVerify.id
            ? { ...item, govtStatus: "Verified" }
            : item
        )
      );
  ...
  const filteredInventory = inventory
    .filter((item) =>
      filterStatus === "All" ? true : item.govtStatus === filterStatus
    )
    .filter((item) => (onlyVerified ? item.govtStatus === "Verified" : true))
    ...
    <div className="bg-slate-50 min-h-screen font-sans antialiased">
      <main className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <header className="mb-10 text-center">
          <h1 className="text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">
            {onlyVerified ? "Verified Produce" : "Inventory Management"}
          </h1>
          <p className="mt-2 text-lg text-slate-600">
            {onlyVerified
              ? "Browse only verified produce."
          ...
          <div className="lg:col-span-2">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {filteredInventory.length > 0 ? (
                    ...
                    <div className="p-5">
                      ...
                      <div className="flex gap-2 mt-5">
                        <button
                          ...
                          className="flex-1 px-4 py-2.5 bg-emerald-600 text-white font-semibold rounded-lg shadow-md hover:bg-emerald-700 transition"
                        >
                          View Details
                        </button>
                      </div>
                ...
                <div className="md:col-span-2 bg-white text-center text-slate-500 py-16 rounded-xl shadow-sm border">
                  No products found.
                </div>
              )}
            </div>
            ...
            <div className="lg:col-span-1">
              <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 text-center sticky top-8">
                ...
                <p className="text-slate-600 mb-6 text-sm">
                  Use your mobile device to view verified products.
                </p>
                <div className="flex justify-center">
                  <div
                    onClick={() => navigate("/verified")}
                    className="cursor-pointer p-1 rounded-lg border-2 border-slate-200 relative hover:scale-105 transition"
                  >
                    <div className="flex justify-center">
                      <QRCodeCanvas
                        value="https://sihfrontend-sigma.vercel.app/verified"
                        size={220}
                        bgColor="#ffffff"
                        fgColor="#000000"
                        level="H"
      ...
      {selected && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-50 p-4 animate-fadeIn">
          <div
            className={`bg-white p-8 rounded-2xl shadow-2xl w-full max-w-md relative transition-transform duration-300 ${
              isClosing ? "animate-scaleOut" : "animate-scaleIn"
            }`}
          >
            <button
              onClick={handleCloseModal}
              className="absolute top-4 right-4 text-slate-400 hover:text-red-500 hover:bg-slate-100 rounded-full p-1 transition"
            ...
            <div className="text-center">
              <img
                src={selected.image}
                alt={selected.name}
                className="w-40 h-40 object-cover rounded-full shadow-lg mb-4 mx-auto border-4 border-white"
              />
              <h3 className="text-3xl font-extrabold text-emerald-600">
                {selected.name}
              </h3>
              <div className="my-2">
                <StatusBadge status={selected.govtStatus} />
              </div>
            </div>
            ...
            <div className="space-y-4 text-slate-700 text-left text-base">
              ...
              <p className="flex items-center gap-3">
                ...
                <strong className="font-medium text-slate-500">Origin:</strong>{" "}
                <span className="ml-auto font-semibold text-slate-800">
                  {selected.origin}
                </span>
              </p>
            ...
            {selected.govtStatus === "Pending" && (
              <div className="mt-8">
                <button
                  onClick={() => handleVerifyProduct(selected)}
                  disabled={isVerifying}
                  className="w-full flex justify-center items-center gap-2 px-6 py-3 bg-emerald-600 text-white font-semibold rounded-lg shadow-md hover:bg-emerald-700 transition disabled:bg-emerald-400 disabled:cursor-not-allowed"
                >
                  {isVerifying ? (
                    <>
